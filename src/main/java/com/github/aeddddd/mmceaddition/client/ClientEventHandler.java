...
public class ClientEventHandler {
    ...
    public static void onItemTooltip(ItemTooltipEvent event) {
        ...
        if (item == Item.getItemFromBlock(RegistryHandler.ME_ASYNC_ITEM_OUTPUT_BUS)) {
            addTooltip(event.getToolTip(), "tooltip.mmceaddition.me_async_item_output_bus");
        } else if (item == Item.getItemFromBlock(RegistryHandler.ME_ASYNC_FLUID_OUTPUT_HATCH)) {
            addTooltip(event.getToolTip(), "tooltip.mmceaddition.me_async_fluid_output_hatch");
        }
    }

    private static void addTooltip(List<String> tooltip, String key) {
