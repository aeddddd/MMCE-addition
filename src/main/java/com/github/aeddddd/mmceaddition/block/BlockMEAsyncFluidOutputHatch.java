...
public class BlockMEAsyncFluidOutputHatch extends BlockMachineComponent {
    ...
    public BlockMEAsyncFluidOutputHatch() {
        super(Material.IRON);
        setTranslationKey(MMCEAddition.MODID + ".me_async_fluid_output_hatch");
        setRegistryName(MMCEAddition.MODID, "me_async_fluid_output_hatch");
        setHardness(2.0f);
        setResistance(10.0f);
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
        ...
        if (!worldIn.isRemote) {
            ...
            if (te instanceof TileMEAsyncFluidOutputHatch) {
                TileMEAsyncFluidOutputHatch hatch = (TileMEAsyncFluidOutputHatch) te;
                playerIn.sendMessage(new TextComponentString(
                        "ME Async Fluid Output Hatch: " + hatch.getBufferedAmount() + " mB buffered"));
            }
        }
        return true;
    }
    ...
    public TileEntity createTileEntity(@Nonnull World world, @Nonnull IBlockState state) {
        return new TileMEAsyncFluidOutputHatch();
    }
    ...
    public void onBlockPlacedBy(World worldIn, BlockPos pos, IBlockState state,
        ...
        TileEntity te = worldIn.getTileEntity(pos);
        if (te instanceof TileMEAsyncFluidOutputHatch && stack.hasTagCompound()) {
            ((TileMEAsyncFluidOutputHatch) te).readBufferFromNBT(stack.getTagCompound());
        }
    }
}
